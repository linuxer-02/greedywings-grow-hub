...
const services = [
  {
    ...
    icon: Globe,
    description:
      "We architect stunning, conversion-focused websites that don't just look exceptional — they perform at the highest level. From landing pages to full e-commerce ecosystems, every pixel is intentional.",
    features: [
      "UI/UX Design",
  ...
  },
  {
    ...
    icon: Video,
    description:
      "We create scroll-stopping video content that captures attention and drives action. Whether it's short-form for social or cinematic commercial production, we bring your vision to life.",
    features: [
      "Short-form Content",
  ...
  },
  {
    ...
    icon: Database,
    description:
      "We build custom software and CRM solutions tailored to your unique operational needs. Streamline workflows, eliminate bottlenecks, and unlock the true potential of your data.",
    features: [
...
export function ServicesShowcase() {
  ...
  return (
    <section
      id="services"
      className="mx-auto flex w-full max-w-[1500px] flex-col items-start justify-start px-5 py-24 md:px-10 md:py-32"
    >
      {/* Header Section */}
      <div className="flex w-full flex-col gap-8 md:flex-row md:items-end md:justify-between lg:gap-16">
        ...
        <div className="max-w-md pb-4">
          <p className="font-mono text-sm leading-relaxed text-muted-foreground/80">
            ...
            every stage of your brand's growth.
          </p>
        </div>
      </div>
          ...
          {services.map((service) => {
            ...
            return (
              <button
                ...
                className={`group relative flex w-full flex-col items-center justify-center overflow-hidden rounded-xl border p-4 text-center transition-all duration-500 hover:border-primary/80 sm:p-6 lg:items-start lg:p-6 lg:text-left xl:p-8 ${
                  isActive
              ...
              >
                ...
                <div className="relative z-10 flex flex-col items-center gap-3 lg:hidden">
                  <div
                    className={`flex h-12 w-12 items-center justify-center rounded-lg transition-colors duration-500 ${isActive ? "bg-primary text-primary-foreground" : "bg-muted text-muted-foreground group-hover:bg-primary/20 group-hover:text-primary"}`}
                  >
                    <Icon className="h-6 w-6" />
                  </div>
                  <span
                    className={`font-mono text-[10px] font-bold tracking-widest uppercase transition-colors duration-500 sm:text-xs ${isActive ? "text-primary" : "text-muted-foreground group-hover:text-primary/80"}`}
                  >
                    {service.shortName}
                  </span>
                </div>
                ...
                <div className="relative z-10 hidden w-full lg:block">
                  <div className="flex w-full items-center justify-between">
                    <div className="flex items-center gap-4">
                      <div
                        className={`flex h-10 w-10 items-center justify-center rounded-lg transition-colors duration-500 ${isActive ? "bg-primary text-primary-foreground" : "bg-muted text-muted-foreground group-hover:bg-primary/20 group-hover:text-primary"}`}
                      >
                        <Icon className="h-5 w-5" />
                      </div>
                      <span
                        className={`font-mono text-xs font-semibold tracking-widest ${isActive ? "text-primary" : "text-muted-foreground group-hover:text-primary/80"}`}
                      >
                        // {service.id}
                      </span>
                    </div>
                    <ArrowRight
                      className={`h-5 w-5 transition-all duration-500 ${isActive ? "text-primary translate-x-0 opacity-100" : "-translate-x-4 text-muted-foreground opacity-0 group-hover:-translate-x-2 group-hover:opacity-60"}`}
                    />
                  </div>

                  <div className="mt-6">
                    <h3
                      className={`font-display text-xl font-bold tracking-tight xl:text-2xl transition-colors duration-500 ${isActive ? "text-foreground" : "text-foreground/80"}`}
                    >
                      {service.title}
                    </h3>
                    <ScrambleText
