...
function InputField({
  ...
  return (
    <div className="flex flex-col gap-1.5">
      <label htmlFor={id} className="font-mono text-[10px] font-bold uppercase tracking-[0.2em] text-foreground/80">
        {label} {required && <span className="text-primary">*</span>}
      </label>
      <div className={`relative flex items-center border-2 transition-colors duration-200 ${focused ? "border-primary" : "border-foreground/30 hover:border-foreground/60"}`}>
        <Icon className={`absolute left-4 h-4 w-4 transition-colors ${focused ? "text-primary" : "text-foreground/60"}`} />
        <input
          id={id}
          type={type}
          value={value}
  ...
  return (
    <div className="flex flex-col gap-1.5">
      ...
      <div className="relative">
        ...
        >
          <span className={`font-sans text-sm font-medium ${selected ? "text-foreground" : "text-foreground/40"}`}>
            {selected ? selected.label : "Select a service…"}
          </span>
          <ChevronDown className={`h-4 w-4 text-foreground/60 transition-transform duration-200 ${open ? "rotate-180" : ""}`} />
        </button>
      ...
      <AnimatePresence mode="wait">
        {state === "success" ? (
          ...
          >
            ...
            <div>
              <h3 className="font-display text-2xl font-black text-foreground">You're booked in!</h3>
          ...
          >
            <div className="grid grid-cols-1 gap-5 sm:grid-cols-2">
              <InputField id="name" label="Full Name" icon={User} value={form.name} onChange={set("name")} placeholder="Your name" required />
              <InputField id="email" label="Email Address" type="email" icon={Mail} value={form.email} onChange={set("email")} placeholder="you@company.com" required />
            </div>

            <div className="grid grid-cols-1 gap-5 sm:grid-cols-2">
              <InputField id="phone" label="Phone Number" type="tel" icon={Phone} value={form.phone} onChange={set("phone")} placeholder="+91 98765 43210" required />
              <ServiceSelect value={form.service} onChange={set("service")} />
            </div>

            <div className="flex flex-col gap-1.5">
              <label htmlFor="message" className="font-mono text-[10px] font-bold uppercase tracking-[0.2em] text-foreground/80">
            ...
            <button
              type="submit"
              disabled={state === "loading" || !form.name || !form.email || !form.phone || !form.service}
            ...
            >
              {state === "loading" ? (
                ...
                <>Book Intro Call <ArrowRight className="h-4 w-4 transition-transform group-hover:translate-x-1" /></>
              )}
            </button>
          </motion.form>
        )}
